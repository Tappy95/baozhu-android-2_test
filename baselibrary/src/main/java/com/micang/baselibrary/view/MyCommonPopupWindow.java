 ...
 *点击外部不消失pop
 */
public abstract class MyCommonPopupWindow {

    protected Context context;
    protected View contentView;
    protected NewPopWindow mInstance;
    public boolean canDismiss = true;// 设为false可以控制dismiss()无参方法不调用 主要是为了点击PopupWindow外部不消失

    public MyCommonPopupWindow(Context c, int layoutRes, int w, int h) {
        context = c;
...
//        initEvent();
        ...
        mInstance = new NewPopWindow(contentView, w, h, true);
    ...
    public NewPopWindow getPopupWindow() {
        return mInstance;
    }
