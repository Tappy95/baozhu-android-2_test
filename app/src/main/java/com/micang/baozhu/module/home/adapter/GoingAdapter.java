...
public class GoingAdapter extends BaseQuickAdapter<GameBean.ListBean, BaseViewHolder> {
    public GoingAdapter(int layoutResId, @Nullable List<GameBean.ListBean> data) {
    ...
    protected void convert(BaseViewHolder helper, GameBean.ListBean item) {
        ...
        if (item != null) {
            ((SwipeMenuLayout) helper.itemView).setIos(true).setLeftSwipe(true);
            ...
            if (tryTag == 1) {
                ...
                helper.setText(R.id.iv_sign_game, "提现任务");
                helper.getView(R.id.iv_sign_game).setVisibility(View.VISIBLE);
            } else {
                helper.setText(R.id.iv_sign_game, "每日红包");
                helper.getView(R.id.iv_sign_game).setVisibility(View.VISIBLE);
            }
            ...
            helper.addOnClickListener(R.id.constraint);
            helper.addOnClickListener(R.id.bt_continue);
        }
    }
}
