...
public class SignMakeXWGameDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void init(Bundle savedInstanceState) {
        ...
        signid = intent.getIntExtra("signid", 0);
        llBack.setOnClickListener(this);
        initWebView();
        gyrosensor = GyrosensorUtils.getInstance(this);
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(SignMakeXWGameDetailActivity.this, new String[]{Manifest.permission.READ_PHONE_STATE}, REQUEST_PHONE_STATE);
        } else {
            getUrl();
            onCallPermission();
        }
    ...
    private void getUrl() {
        ...
        HttpUtils.getqdGameUrl(signid, gameId, moblie).enqueue(new Observer<BaseResult>() {
            @Override
    ...
    public void onCallPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {   //判断当前系统的SDK版本是否大于23
            ...
            if (!(checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED)) { //如果当前申请的权限没有授权
                ...
                //请求权限
                ActivityCompat.requestPermissions(SignMakeXWGameDetailActivity.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, REQUEST_WRITE_STORAGE);
            } else {//已经授权了就走这条分支
            ...
            case REQUEST_INSTALL_STATE: {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (Build.VERSION.SDK_INT >= 26) {
                        AppUtil.jumpInstallPermissionSetting(SignMakeXWGameDetailActivity.this, REQUEST_INSTALL_UNKNOW_SOURCE);
                    }
                }
                break;
            }
...
//                    Toast.makeText(this, "你不给权限我就不好干事了啦", Toast.LENGTH_SHORT).show();
    ...
    private void initWebView() {
        ...
        webView.addJavascriptInterface(SignMakeXWGameDetailActivity.this, "android");
    }
    ...
    public void CheckInstall(String packageName) {
        boolean isInstalled = AppUtil.isApkInstalled(SignMakeXWGameDetailActivity.this, packageName);
        if (isInstalled) {
            ...
            if (status == -1 || (status == DownloadManager.STATUS_SUCCESSFUL && !file.exists())) { // 下载  条件： 不在下载中 或者下载完成但是文件已被删除
                ...
                if (sdkVersion >= 26 && Build.VERSION.SDK_INT >= 26) {
                    ...
                    if (b) {
                        ...
                        ActivityCompat.requestPermissions(SignMakeXWGameDetailActivity.this, new String[]{Manifest.permission.REQUEST_INSTALL_PACKAGES}, REQUEST_INSTALL_STATE);
                    }
                } else {
                    AppUtil.installAPK(mContext, new File(downloadPath));
                }
...
//        }
    ...
    protected void installAPK(File file, String apkName) {
        ...
        Uri uri;
        if (SignMakeXWGameDetailActivity.this == null || SignMakeXWGameDetailActivity.this.isFinishing()) {
            return;
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            new RxPermissions(SignMakeXWGameDetailActivity.this).request(Manifest.permission.REQUEST_INSTALL_PACKAGES).subscribe(new Consumer<Boolean>() {
                @Override
                public void accept(Boolean aBoolean) throws Exception {
                    if (aBoolean) {
                        ...
                        Toast.makeText(SignMakeXWGameDetailActivity.this, "未授权权限，部分功能不能使用", Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            File newPath = new File(SignMakeXWGameDetailActivity.this.getFilesDir().getPath() + "/downloads");
            if (!newPath.exists()) {
