...
public class EarlyClockRulesActivity extends BaseActivity {
    ...
    @Override
    public int layoutId() {
        return R.layout.activity_early_clock_rules;
    }
...
//                .setMainFrameErrorView(R.layout.fragment_home,R.id.rl_challenge)
                .createAgentWeb()
                .ready()
                .go(RetrofitUtils.H5url + "/clockRules.html?token=" + token + "&imei=" + imei);
    ...
    private class AndroidInterface {
        public AndroidInterface(AgentWeb agentWeb, EarlyClockRulesActivity activity) {
        }

        @JavascriptInterface
        public void over() {
            EarlyClockRulesActivity.this.finish();
        }
    }
}
