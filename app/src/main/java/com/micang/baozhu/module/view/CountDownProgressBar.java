 ...
 * @author
 * @version 1.0
 * @Package com.dizoo.invite.module.view
 * @time 2019/4/25 16:29
 * @describe describe
 */
public class CountDownProgressBar extends View {
     ...
     * 渐变圆周颜色数组
     */
    private int[] colorArray = new int[]{Color.parseColor("#FFEB37"),
            Color.parseColor("#FFEB37"), Color.parseColor("#FFEB37")};
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (attr) {
                case R.styleable.CountDownProgressBar_countDown_firstColor:
                    firstColor = ta.getColor(attr, Color.rgb(255, 235, 55)); // 默认底色为亮灰色
                    break;
                ...
                case R.styleable.CountDownProgressBar_countDown_centerTextSize:
                    centerTextSize = ta.getDimensionPixelSize(attr, DensityUtil.sp2px(getContext(), 12)); // 默认中间文字字体大小为40dp
                    break;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawBootomCircle(canvas, center, radius);
        drawCircle(canvas, center, radius); // 绘制进度圆弧
        drawText(canvas, center);
    }

    private void drawBootomCircle(Canvas canvas, int center, int radius) {
        circlePaint.setShader(null); // 清除上一次的shader
        circlePaint.setColor(Color.parseColor("#FDC72F")); // 设置底部圆的颜色
        circlePaint.setStyle(Paint.Style.FILL); // 设置绘制的圆为空心
    ...
    private void drawCircle(Canvas canvas, int center, int radius) {
        circlePaint.setShader(null); // 清除上一次的shader
        circlePaint.setColor(Color.parseColor("#FDC72F")); // 设置底部圆环的颜色，这里使用第一种颜色
