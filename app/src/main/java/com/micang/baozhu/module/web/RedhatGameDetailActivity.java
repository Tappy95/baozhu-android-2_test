...
@BindEventBus
public class RedhatGameDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    @Override
    public int layoutId() {
        return R.layout.activity_xwgame_detail;
    }
    ...
    public void init(Bundle savedInstanceState) {
        ...
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(RedhatGameDetailActivity.this, new String[]{Manifest.permission.READ_PHONE_STATE}, REQUEST_PHONE_STATE);
        } else {
            openUrl();
            onCallPermission();
        }
    ...
    public void onCallPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {   //判断当前系统的SDK版本是否大于23
            ...
            if (!(checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED)) { //如果当前申请的权限没有授权
                ...
                //请求权限
                ActivityCompat.requestPermissions(RedhatGameDetailActivity.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, REQUEST_WRITE_STORAGE);
            } else {//已经授权了就走这条分支
            ...
            case REQUEST_INSTALL_STATE: {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (Build.VERSION.SDK_INT >= 26) {
                        AppUtil.jumpInstallPermissionSetting(RedhatGameDetailActivity.this, REQUEST_INSTALL_UNKNOW_SOURCE);
                    }
                }
                break;
            }
...
//                    Toast.makeText(this, "你不给权限我就不好干事了啦", Toast.LENGTH_SHORT).show();
    ...
    private void initWebView() {
        ...
        webView.addJavascriptInterface(RedhatGameDetailActivity.this, "android");
    }
    ...
    public void getAPP(String packageName) {
        boolean isInstalled = AppUtil.isApkInstalled(RedhatGameDetailActivity.this, packageName);
        if (isInstalled) {
            ...
            if (status == -1 || (status == DownloadManager.STATUS_SUCCESSFUL && !file.exists())) { // 下载  条件： 不在下载中 或者下载完成但是文件已被删除
                ...
                if (sdkVersion >= 26 && Build.VERSION.SDK_INT >= 26) {
                    ...
                    if (b) {
                        ...
                        ActivityCompat.requestPermissions(RedhatGameDetailActivity.this, new String[]{Manifest.permission.REQUEST_INSTALL_PACKAGES}, REQUEST_INSTALL_STATE);
                    }
                } else {
                    AppUtil.installAPK(mContext, new File(downloadPath));
                }
...
//        }
    ...
    protected void installAPK(File file, String apkName) {
        ...
        Uri uri;
        if (RedhatGameDetailActivity.this == null || RedhatGameDetailActivity.this.isFinishing()) {
            return;
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {

            new RxPermissions(RedhatGameDetailActivity.this).request(Manifest.permission.REQUEST_INSTALL_PACKAGES).subscribe(new Consumer<Boolean>() {
                @Override
                public void accept(Boolean aBoolean) throws Exception {
                    if (aBoolean) {
                        ...
                        Toast.makeText(RedhatGameDetailActivity.this, "未授权权限，部分功能不能使用", Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            File newPath = new File(RedhatGameDetailActivity.this.getFilesDir().getPath() + "/downloads");
            if (!newPath.exists()) {
    ...
    private void initbackPopWindow() {
        ...
        backPop = new MyCommonPopupWindow(this, R.layout.game_back_pop_new, ViewGroup.LayoutParams.WRAP_CONTENT,
            ...
            protected void initView() {
                ...
                Glide.with(RedhatGameDetailActivity.this).load(R.drawable.icon_bg_changegame).into(ivwelfare);
                ...
                Glide.with(RedhatGameDetailActivity.this).load(bean.icon).into(ivGamePic);
                ...
                ivwelfare.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (EmptyUtils.isEmpty(fuLiHui)) {
                            ...
                            Intent intent = new Intent(RedhatGameDetailActivity.this, WelfareActivity.class);
    ...
    private void getUrl() {
        if ("PCDD".equals(interfaceName)) {
            HttpUtils.toPlay(moblie, gameId).enqueue(new Observer<BaseResult>() {
                ...
                public void onSuccess(BaseResult response) {
                    ...
                    Intent intent = new Intent(RedhatGameDetailActivity.this, NextPCddGameDetailActivity.class);
        ...
        if ("MY".equals(interfaceName) || "bz-Android".equals(interfaceName)) {
            HttpUtils.toPlay(moblie, gameId).enqueue(new Observer<BaseResult>() {
                ...
                public void onSuccess(BaseResult response) {
                    String url = (String) response.data;
                    Intent intent = new Intent(RedhatGameDetailActivity.this, NextMYGameDetailsActivity.class);
                    intent.putExtra("URLS", url);
                    intent.putExtra("bean", gameBean);
                    backPop.getPopupWindow().dismiss2();
                    startActivity(intent);
        ...
        if ("xw-Android".equals(interfaceName)) {
            HttpUtils.toPlay(moblie, gameId).enqueue(new Observer<BaseResult>() {
                ...
                public void onSuccess(BaseResult response) {
                    String url = (String) response.data;
                    Intent intent = new Intent(RedhatGameDetailActivity.this, NextXWGameDetailActivity.class);
                    intent.putExtra("URLS", url);
                    intent.putExtra("bean", gameBean);
                    backPop.getPopupWindow().dismiss2();
                    startActivity(intent);
        ...
        HttpUtils.recommendGame().enqueue(new Observer<BaseResult<ChangeBean>>() {
            ...
            public void onSuccess(BaseResult response) {
                ...
                if (EmptyUtils.isNotEmpty(gameBean)) {
                    ...
                    Glide.with(RedhatGameDetailActivity.this).load(gameBean.icon).into(ivGamePic);
